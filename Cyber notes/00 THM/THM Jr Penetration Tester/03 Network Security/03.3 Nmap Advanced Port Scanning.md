...
Spoofing isn't often available
--> Idle/zombie scan: **idle system** connected to network, nmap makes probes **appear** to be coming from said system, then checks for **indicators** system received response

`-sI ZOMBIE_IP TARGET_IP`

1. Trigger idle system --> record IP ID
2. Send SYN packet to TCP port on target, spoofed with idle IP --> IP ID incremented by 1 if port is open
3. Trigger idle system --> record new IP ID to compare with 1. (IP ID incremented by 1 again so in the end, incremented by 1 means closed/filtered port, 2 open port)

___
### More details
[[#Table of contents|Back to the top]]

`--reason`: details about reasoning and conclusions
`-v`, `-vv`: verbose
`-d`, `-dd`: debugging (tons of details)